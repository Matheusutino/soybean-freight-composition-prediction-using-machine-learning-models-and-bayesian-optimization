...
class BayesianOptimizer:
    def __init__(self, 
        ...
        self.metric_to_optimize = 'f1' if self.task_type == 'classification' else 'rsme'
        self.all_metrics = []
    ...
    def objective(self, trial: optuna.trial):
        ...
        for metric in score.keys():
            mean_metrics[metric] = [result[metric] for result in fold_scores]
            mean_metrics[metric] = np.mean(mean_metrics[metric]) 

        self.all_metrics.append(mean_metrics)
    ...
    def get_model(self, trial: optuna.trial):
        if self.model_name == 'KNN':
            ...
            model = lgb.LGBMClassifier(random_state=self.seed, n_jobs = -1) if self.task_type == 'classification' else lgb.LGBMRegressor(random_state=self.seed, n_jobs = -1)
