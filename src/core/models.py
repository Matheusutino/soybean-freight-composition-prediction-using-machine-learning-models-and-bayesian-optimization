...
class Model:
    ...
    def get_model(model_name:str, 
        ...
        if model_name == 'KNN':
            ...
            model = SVC(**params, random_state=seed) if task_type == 'classification' else SVR(**params, random_state=seed)
            ...
            model = XGBClassifier(**params, eval_metric='logloss', seed = seed) if task_type == 'classification' else XGBRegressor(**params, seed = seed)
            ...
            model = tf.keras.Sequential([
                ...
                *[
                    tf.keras.layers.Dense(num_units, activation='relu'),
                    tf.keras.layers.Dropout(dropout_rate, seed = seed)
                 ] * num_layers,
                tf.keras.layers.Dense(num_classes, activation='softmax' if task_type == 'classification' else 'linear')
            ])
